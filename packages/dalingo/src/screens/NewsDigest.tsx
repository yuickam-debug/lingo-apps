...
const BUNDLE_URL =
  'https://raw.githubusercontent.com/yuickam-debug/lingo-apps/main/packages/dalingo/src/content/news/da-news-bundle.json';
const CACHE_KEY = 'lingo_news_da_v1';
...
function mergeWeeks(base: WeekData[], remote: WeekData[]): WeekData[] {
  ...
  for (const w of remote) map.set(w.weekOf, w);
  ...
  return (
    <div>
      <div className="screen-header">
        <h2 className="screen-title">Nyheder</h2>
      ...
      {syncStatus !== 'idle' && (
        <div style={{ padding: '0 20px 8px', fontSize: '11px', color: 'var(--ink-2)', display: 'flex', alignItems: 'center', gap: '5px' }}>
          {syncStatus === 'fetching' && (
            <>
              <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: '50%', background: 'var(--accent)', opacity: 0.6, animation: 'reviewPulse 1s ease-in-out infinite' }} />
              Opdaterer…
            </>
          )}
          {syncStatus === 'synced' && (
            <>
              ...
              {' '}Opdateret {syncedAt ? new Date(syncedAt).toLocaleTimeString('da-DK', { hour: '2-digit', minute: '2-digit' }) : ''}
            </>
          )}
          {syncStatus === 'cached' && (
            <>
              <span>↺</span>
              {' '}Cachelagret {syncedAt ? new Date(syncedAt).toLocaleTimeString('da-DK', { hour: '2-digit', minute: '2-digit' }) : ''}
            </>
          )}
          {syncStatus === 'error' && (
            <>
              <span style={{ color: '#E53E3E' }}>✕</span>
              {' '}Opdatering mislykkedes — viser offline-indhold
            </>
          )}
        </div>
      )}
      ...
      <div className="news-week-nav">
        ...
        <span className="news-week-label">
          {isLatest ? 'Denne uge' : `Uge ${week.weekOf}`}
        </span>
        <button
          className="news-week-btn"
          onClick={() => setWeekIndex((i) => i + 1)}
      ...
      <p className="section-header">
        {isLatest ? 'Denne uge' : `Uge fra ${formatWeekHeader(week.weekOf)}`}
      </p>
          ...
          >
            <div className="story-item-info">
              <div className="news-card-tags">
                ...
                {isLatest && <span className="news-new-badge">NY</span>}
              </div>
              ...
              <div className="story-item-meta">
                ...
                <span>{article.sentences.length} sætninger</span>
              </div>
            </div>
